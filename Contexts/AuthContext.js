...
const AuthProvider = ({children}) => {
  ...
  const signIn = async authCode => {
    try {
      ...
      Toast.show({
        type: 'error',
        text1: 'Ошибка логина',
        text2: 'Попробуйте снова или перезагрузите приложение!',
      });
    } finally {
      setLoading(false);
    }
  ...
  async function loadStorageData() {
    try {
      setLoading(true);
      const lastTimeConfigUpdated = await AsyncStorage.getItem(
        'lastTimeConfigUpdated',
      );
      ...
      const shouldUpdateData = shouldUpdateAuthData(
        parsedLastTimeConfigUpdated,
      );
